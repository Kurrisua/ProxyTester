...
func GetSocksFromQuake(quake QUAKEConfig) {
	...
	for _, item := range arr {
		...
		addSocks(ip+":"+strconv.FormatFloat(port, 'f', -1, 64), "quake")
	}
}
...
func GetSocksFromFofa(fofa FOFAConfig) {
	...
	for _, itemArray := range array {
		...
		addSocks(itemSlice[0].(string)+":"+itemSlice[1].(string), "FOFA")
	}
...
func GetSocksFromHunter(hunter HUNTERConfig) {
	...
	for i := 1; i <= end; i++ {
		...
		for _, item := range arr {
			...
			port := itemMap["port"].(float64)
			exeData++
			addSocks(ip+":"+strconv.FormatFloat(port, 'f', -1, 64), "HUNTER")
		}
...
func GetSocksFromFile(socksFileName string) {
	...
	if !os.IsNotExist(err) {
		fmt.Println("***当前目录下存在 " + socksFileName + ", 将尝试读取代理及来源信息***")
		...
		fmt.Println(socksFileName + " 文件不存在，将从网络测绘平台获取")
	}
}
