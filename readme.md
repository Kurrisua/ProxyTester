...
ProxyTester 是一个代理采集、检测、评分、入库、查询的可视化平台。当前版本已经从单纯的“代理可用性检测工具”升级为代理安全研究平台的基础版：系统不仅判断代理是否可用，还会记录检测批次、漏斗阶段记录、安全事件、证据、资源观测、证书观测、蜜罐访问摘要和地理聚合结果，帮助把代理从“可用/不可用”的资源对象升级为“正常/可疑/恶意”的行为研究对象。
